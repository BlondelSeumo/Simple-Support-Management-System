    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <span class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                ...
                <a class="text-white">{{ __('faq.add') }}</a>
            </span>
        </div>
                ...
                <form method="POST" action="<?=route('admin.faq.store')?>">
                    ...
                    <div class="card shadow mb-4">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="status" class="form-control @error('status') is-invalid @enderror">
                                        ...
                                        @foreach(trans('faq_status') as $statusKey=> $status)
                                            <option value="{{ $statusKey }}" {{ (old('status') == $statusKey) ? 'selected' : '' }}>{{ $status }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                    <div class="invalid-feedback">
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary">{{ __('faq.create_faq') }}</button>
                        </div>
                    </div>
                </form>
            </div>
